...
const handleBid = () => {
  ...
  return (
    <div className="bg-blue-900 min-h-screen p-8 flex flex-col items-center space-y-6">
      <h1 className="text-5xl font-extrabold text-white mb-8">All Tasks</h1>
      ...
      {myJobs.length === 0 ? (
        ...
        myJobs.map((job) => (
          ...
          >
            <h2 className="text-2xl font-bold mb-3 border-b border-blue-600 pb-2">{job.name}</h2>
            ...
            <p><span className="font-semibold">Additional Req:</span> {job.attachment}</p>
...
>
  Details
              ...
              className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg">Bid</button>
            </div>
          </div>
        ))
      )}
